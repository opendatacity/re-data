...
// Livestream test
var streamURLs = {
	"test-rp15-location-5591": "http://delive.artestras.cshls.lldns.net/artestras/contrib/delive.m3u8",
	"test-rp15-location-5929": "http://delive.artestras.cshls.lldns.net/artestras/contrib/frlive.m3u8"
};
	...
	require('../lib/json_requester').get(
		...
		function (result) {
			...
			speakerList.forEach(function (speaker) {
				...
				var speakerImageURL = null;
				if (typeof(speaker.image) == "string") {
					speakerImageURL = speaker.image;
				} else if (speaker.image["src"]) {
					speakerImageURL = speaker.image["src"];
				}

				var entry = {
					'id': eventId + '-speaker-'+speaker.uid,
					'name': speakerName,
					'photo': speakerImageURL,
					'url': speaker.uri,
		...
		//             }
			...
			sessionList.forEach(function (session) {
				...
				if (session.video) {	
					var videoURLFunction = function (videoURL) {
					...
					console.log("session.video: ", session.video);
					if (typeof(session.video) == "string") {
						videoURLFunction(session.video);
					} else {
						session.video.forEach(videoURLFunction);
					} 
				}
