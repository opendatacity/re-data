...
var scraper = new scrapyard({
	cache: path.resolve(__dirname, "storage"), 
	debug: true,
	timeout: 300000,
	retries: 5,
	connections: 10
...
scraper.scrape('http://13.re-publica.de/rp13-schedule.xml','xml','utf8',function(err, xml){
	...
	xml.schedule.day.forEach(function(day){
		...
		/* push day */
		var day_id = [_event, "day", day.$.index].join("-");
		...
		day.room.forEach(function(room){
			...
			/* area id */
			var area_id = [_event, "area", _nickname(room.$.name)].join("-");
			...
			room.event.forEach(function(event){
				...
				/* make id for session */
				var session_id = [_event, "session", event.$.id].join("-");
				...
				event.persons[0].person.forEach(function(person){
					...
					var speaker_id = [_event, "speaker", person.$.id].join("-")
				...
				data.push({
					...
					"speakers": speakers,
					"revision": 1,
					"last-modified": moment().format("YYYY-MM-DD[T]HH:mm:ssZ")
				});
	...
	xml.schedule.speakers[0].speaker.forEach(function(speaker){
		...
		var speaker_id = [_event, "speaker", speaker.$.persons].join("-");
		data.push({
			"id": speaker_id,
			"event": _event,
			"type": "speaker",
	...
	fs.writeFileSync(path.resolve(__dirname, "../data/"+_event+"-data.json"), JSON.stringify(data,null,'\t'));
